...
job_table_spliced.to_csv("job_table_spliced.csv", sep=',', index=False, encoding='utf-8')


tempo_worn_spliced = tempo_worn[["facility_id", "institution_id", "rr_rule", "start_date", "report_file_name"]]
tempo_worn_spliced.to_csv("tempo.csv", sep=',', index=False, encoding='utf-8')
...
sorted_tab.to_csv("active_sites.csv", sep=',', index=False, encoding='utf-8')


def merge_csvs():
    active_jobs = pd.merge(sorted_tab, job_table_spliced, how="left")
    active_jobs.to_csv("active_jobs.csv", sep=',', index=False, encoding='utf-8')

    active_tempos = pd.merge(sorted_tab, tempo_worn_spliced, how="left")
    active_tempos.to_csv("active_tempos.csv", sep=',', index=False, encoding='utf-8')
...
def create_unique_report_names():
    ...
    job_table_spliced = job_table[['facility_id', "file_name", "cron_expression"]]

    tempo_worn_spliced = tempo_worn[["facility_id", "institution_id", "rr_rule", "report_file_name"]]
    ...
    for x in unique_report_names:
        unique_report_names[unique_report_names.index(x)] = x.strip().replace(".pdf", "").title().replace("_", " ")
